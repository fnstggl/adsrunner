...
- Do NOT include any <style> block or @font-face declarations — fonts are injected automatically after your response.
...
def render_text_overlay(
    ...
    try:
        ...
        # 540×675 JPEG ~12KB vs full 1080×1350 PNG ~2-4MB — 40-80x smaller payload.
        ...
        # Step 2: Fetch fonts (module-level cache — one network round-trip per process)
        fonts = _fetch_fonts()
        font_face_css = _build_font_face_css(fonts)

        # Step 3: Ask Claude to generate the HTML overlay.
        # Font CSS is NOT sent to Claude — injected into the HTML after generation.
        # This removes ~172KB of base64 from the prompt, cutting API time dramatically.
        ...
        # Step 4: Inject @font-face CSS into <head> before Playwright renders.
        # Chromium decompresses WOFF2 natively in milliseconds.
        if font_face_css:
